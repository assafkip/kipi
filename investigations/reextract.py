...
def reextract_report(conn, report_id: int, raw_text: str) -> dict:
    ...
    for e in extractor.extract_all(raw_text or ""):
        ...
        if not existed:
            ...
            conn.execute("UPDATE entities SET entity_type = ? WHERE id = ?", (e.entity_type, eid))
            retyped += 1
            by_type[e.entity_type] = by_type.get(e.entity_type, 0) + 1
...
def run(conn, case: str | None = None) -> dict:
    ...
    for r in rows:
        out = reextract_report(conn, r["id"], r["raw_text"])
        total["reports"] += 1
